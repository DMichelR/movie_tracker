...
class FavoritesFragment : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        binding.titleText.text = "Películas Favoritas"
        ...
        adapter = MovieAdapter(
            onMovieClick = { movie ->
                findNavController().navigate(
                    FavoritesFragmentDirections.actionFavoritesFragmentToMovieDetailFragment(movie.id)
                )
            },
    ...
    private fun observeViewModel() {
        viewModel.favoriteMovies.observe(viewLifecycleOwner) { state ->
            when (state) {
                is UiState.Loading -> {
